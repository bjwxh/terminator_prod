...
function updateUI(state) {
    ...
    // 9.5. Option Book
    updateOptionBook(state.option_book, state.spx);
...
function parseOccSymbol(symbol) {
    ...
    const root = parts[0];
    const date = code.slice(code.length - 15, code.length - 9); // YYMMDD
    const sideChar = code.charAt(code.length - 9);
    const strikeStr = code.slice(code.length - 8);
    const strike = parseFloat(strikeStr) / 1000.0;
    const side = sideChar === 'C' ? 'CALL' : 'PUT';
    
    return { root, date, strike, side };
}

function isSpx0DteOption(symbol) {
    const parsed = parseOccSymbol(symbol);
    if (!parsed) return false;
    if (parsed.root !== 'SPX' && parsed.root !== 'SPXW') return false;
    if (!lastServerTs) return true; // Fallback if server time not received yet
    const todayYymmdd = lastServerTs.slice(2, 4) + lastServerTs.slice(5, 7) + lastServerTs.slice(8, 10);
    return parsed.date === todayYymmdd;
}

function isSpx0DteOrder(symbolStr) {
    if (!symbolStr) return false;
    const symbols = symbolStr.split(',').map(s => s.trim());
    if (symbols.length === 0) return false;
    return symbols.every(sym => isSpx0DteOption(sym));
}
...
function updatePositionTable(livePos, simPos) {
    ...
    const filteredSimPos = (simPos || []).filter(p => isSpx0DteOption(p.symbol));
    const filteredLivePos = (livePos || []).filter(p => isSpx0DteOption(p.symbol));

    const merged = {};
    filteredSimPos.forEach(p => {
        const key = `${p.symbol}-${p.strike}-${p.side}`;
        const qty = p.qty !== undefined ? p.qty : p.quantity;
        merged[key] = {
            strike: p.strike, side: p.side, symbol: p.symbol,
            qty_sim: qty, qty_live: 0,
            delta: p.delta, bid: p.bid, ask: p.ask
        };
    });
    filteredLivePos.forEach(p => {
        const key = `${p.symbol}-${p.strike}-${p.side}`;
        const qty = p.qty !== undefined ? p.qty : p.quantity;
        if (merged[key]) {
            merged[key].qty_live = qty;
...
function updateOptionBook(optionBook, spx) {
    if (!optionBook) return;
    lastOptionBookData = optionBook;
    lastSpxPrice = spx;

    const tbody = document.getElementById('option-book-tbody');
    if (!tbody) return;

    tbody.innerHTML = '';

    const selectEl = document.getElementById('option-book-rows-select');
    const limit = selectEl ? selectEl.value : '20';
    let displayBook = optionBook;

    if (limit !== 'all') {
        const count = parseInt(limit, 10);
        const half = count / 2;

        if (typeof spx === 'number' && spx > 0 && optionBook.length > 0) {
            let closestIdx = 0;
            let minDiff = Infinity;
            for (let i = 0; i < optionBook.length; i++) {
                let diff = Math.abs(optionBook[i].strike - spx);
                if (diff < minDiff) {
                    minDiff = diff;
                    closestIdx = i;
                }
            ...
            let end = Math.min(optionBook.length, closestIdx + half);

            if (start === 0) {
                end = Math.min(optionBook.length, count);
            } else if (end === optionBook.length) {
                start = Math.max(0, optionBook.length - count);
            }

            displayBook = optionBook.slice(start, end);
        } else {
            const mid = Math.floor(optionBook.length / 2);
            let start = Math.max(0, mid - half);
            let end = Math.min(optionBook.length, mid + half);
            if (start === 0) {
                end = Math.min(optionBook.length, count);
            } else if (end === optionBook.length) {
                start = Math.max(0, optionBook.length - count);
    ...
    // console.log(`Option book updated. Limit: ${limit}, displaying ${displayBook.length}/${optionBook.length} rows. SPX: ${spx}`);
    ...
    displayBook.forEach(row => {
        ...
        tr.innerHTML = `
            ...
            <td style="text-align: center; font-weight: bold; color: var(--primary);">${fmtStrike(row.strike)}</td>
...
function initOptionBookRowsSelect() {
    const selectEl = document.getElementById('option-book-rows-select');
    if (selectEl) {
        console.log("Option Book rows select initialized successfully");
        selectEl.addEventListener('change', () => {
            console.log(`Option Book rows selection changed to: ${selectEl.value}`);
            if (lastOptionBookData !== null) {
                updateOptionBook(lastOptionBookData, lastSpxPrice);
            }
        });
    } else {
        console.warn("Option Book rows select element NOT found!");
    }
}

initCharts();
initMuteUI();
initOptionBookRowsSelect();
connect();

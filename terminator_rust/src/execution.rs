...
use crate::parser::{is_0dte_spx, parse_occ_symbol};
use crate::strategy::{OptionLeg, Trade};
...
impl ExecutionClient {
    ...
    pub async fn get_account_hash(&self) -> Result<String> {
            ...
            if let Some(num) = acc.get("accountNumber").and_then(|v| v.as_str()) {
                if num == account_id {
                    if let Some(hash) = acc.get("hashValue").and_then(|v| v.as_str()) {
                        info!(
                            "Successfully resolved account hash for {}: {}",
                            account_id, hash
                        );
        ...
        anyhow::bail!(
            "Could not resolve account hash for account ID: {}",
            account_id
        )
    }
    ...
    pub async fn place_order(
        ...
        info!(
            "Transmitting REST Order Placement: {}",
            serde_json::to_string(&order_body)?
        );
        ...
        if status == reqwest::StatusCode::CREATED || status.is_success() {
            ...
            let body = response.text().await.unwrap_or_default();
            error!(
                "REST order placement failed with status {}: {}",
                status, body
            );
    ...
    pub async fn cancel_order(&self, account_hash: &str, order_id: &str) -> Result<bool> {
        ...
        if response.status().is_success() {
            ...
            let body = response.text().await.unwrap_or_default();
            warn!(
                "Cancellation request rejected for ID {}: {}",
                order_id, body
            );
            Ok(false)
        }
    ...
    pub async fn chase_order(
        ...
        info!(
            "Chasing order {} from ${} to ${:.2} (Mark: {:.2})",
            order_id, old_price, new_abs_price, mark
        );
        ...
        if response.status().is_success() {
            ...
            let body = response.text().await.unwrap_or_default();
            error!(
                "REST order replacement failed with status {}: {}",
                status, body
            );
    ...
    pub async fn get_working_orders(&self, account_hash: &str) -> Result<Vec<Value>> {
        ...
        let now_utc = Utc::now();
        let from_time = (now_utc - chrono::Duration::days(60))
        ...
        info!(
            "Schwab GET /orders returned {} total orders (before filter).",
            orders.len()
        );

        let flattened = flatten_orders(orders);
        let active_orders: Vec<Value> = flattened
            .into_iter()
            .filter(|o| {
    ...
    pub async fn get_today_filled_orders(
        &self,
        account_hash: &str,
        ...
        let start_of_day_ct = now_ct
            ...
            .unwrap();
        let from_time = (start_of_day_ct - chrono::Duration::hours(24))
            .with_timezone(&Utc)
            .to_rfc3339_opts(chrono::SecondsFormat::Millis, true);
        let to_time = now_utc.to_rfc3339_opts(chrono::SecondsFormat::Millis, true);
        ...
        for order in flattened {
            ...
            if let Some(close_time_str) = order.get("closeTime").and_then(|v| v.as_str()) {
                if let Ok(close_dt) =
                    chrono::DateTime::parse_from_rfc3339(&close_time_str.replace("Z", "+00:00"))
                {
            ...
            if let Some(leg_array) = legs {
                ...
                for leg in leg_array {
                    ...
                    let instruction = leg
                        ...
                        .unwrap_or("");
                    let quantity =
                        leg.get("quantity").and_then(|v| v.as_f64()).unwrap_or(0.0) as i32;
                ...
                if !option_legs.is_empty() {
                    let mut executed_price =
                        order.get("price").and_then(|v| v.as_f64()).unwrap_or(0.0);
                    let (_, is_credit_structural) =
                        crate::strategy::classify_order_type(&option_legs);

                    let mut found_execution_legs = false;
                    let mut total_qty = 0.0;
                    let mut weighted_price_sum = 0.0;
                    if let Some(activities) = order
                        .get("orderActivityCollection")
                        .and_then(|v| v.as_array())
                    {
                        for activity in activities {
                            if activity.get("activityType").and_then(|v| v.as_str())
                                == Some("EXECUTION")
                            {
                                let qty = activity
                                    .get("quantity")
                                    .and_then(|v| v.as_f64())
                                    .unwrap_or(0.0);
                                if let Some(exec_legs) =
                                    activity.get("executionLegs").and_then(|v| v.as_array())
                                {
                                    let mut net_price = 0.0;
                                    for exec_leg in exec_legs {
                                        if let Some(leg_id) =
                                            exec_leg.get("legId").and_then(|v| v.as_i64())
                                        {
                                            if let Some(price) =
                                                exec_leg.get("price").and_then(|v| v.as_f64())
                                            {
                                                let mut instruction = "";
                                                if let Some(leg_collection) = order
                                                    .get("orderLegCollection")
                                                    .and_then(|v| v.as_array())
                                                {
                                                    for l in leg_collection {
                                                        if l.get("legId").and_then(|v| v.as_i64())
                                                            == Some(leg_id)
                                                        {
                                                            instruction = l
                                                                .get("instruction")
                    ...
                    if !found_execution_legs && status != "FILLED" {
                        continue;
                    }

                    let requested_base_qty = option_legs
                        .first()
                        .map(|l| l.quantity.abs() as f64)
                        .unwrap_or(1.0)
                        .max(1.0);

                    let base_qty = if found_execution_legs && total_qty > 0.0 {
                        total_qty
                    } else {
                        requested_base_qty
                    };

                    let scale_factor = base_qty / requested_base_qty;
                    for leg in &mut option_legs {
                        leg.quantity = (leg.quantity as f64 * scale_factor).round() as i32;
                    }

use terminator_rust::strategy::{OptionLeg, get_smart_chunks, classify_order_type};
...
fn main() {
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        let (struct_type, is_credit) = classify_order_type(chunk);
        println!("Chunk {}: struct_type={}, is_credit={:?}, num_legs={}", i, struct_type, is_credit, chunk.len());
    }
}
